...
import { getDefined, getNonNull } from 'app/shared/utils';
...
export class FractalAnimationDirective implements OnInit, OnDestroy {
  ...
  @Input('canvasWidth') _canvasWidth?: number;
  get canvasWidth(): number {
    return getDefined(this._canvasWidth);
  }
  @Input('canvasHeight') _canvasHeight?: number;
  get canvasHeight(): number {
    return getDefined(this._canvasHeight);
  }

  @Input('mainFractalRadius') _mainFractalRadius?: number;
  get mainFractalRadius(): number {
    return getDefined(this._mainFractalRadius);
  }
  @Input('mainFractalCenterY') _mainFractalCenterY?: number;
  get mainFractalCenterY(): number {
    return getDefined(this._mainFractalCenterY);
  }
  ...
  ngOnInit(): void {
    const context = getNonNull(this.canvas.getContext('2d'));
    ...
    const cachedBackgroundContext = getNonNull(cachedBackgroundCanvas.getContext('2d'));
    ...
    const timeBetweenSnowFlakeSpawns =
      1 / (this.numberOfSnowFlakesToSpawnPerYPerSecond * this.canvasWidth);
    ...
    cachedBackgroundContext.fillRect(
      0,
      0,
      cachedBackgroundCanvas.width,
      cachedBackgroundCanvas.height,
    );
    ...
    const doFrame = (timeStamp: number) => {
      ...
      if (!donePerformanceCheck && !doingPerformanceCheck && deltaTime > 0.035) {
        this.logger.log(
          PerfLogChannel,
          'Pre perf check, clearing slow frames, deltaTime: ' + deltaTime,
        );
      ...
      if (!doingPerformanceCheck && !donePerformanceCheck) {
        ...
        mainFractal.initForPerformanceChecks(
      ...
      if (doingPerformanceCheck) {
        if (finalizingPerformanceCheck) {
          if (deltaTime > maxFrameLength) {
            this.logger.log(
              PerfLogChannel,
              'Got slow frame during finalization, deltaTime: ' + deltaTime,
            );
            finalizingPerformanceCheckCount = 0;
            mainFractal.reduceFractalIterations(this.logger);
          } else if (
            finalizingPerformanceCheckCount ===
            this.performanceCheckLoopFinalizationIterations - 1
          ) {
            ...
            this.logger.log(
              PerfLogChannel,
              'Passed finalization and finished checks, deltaTime: ' + deltaTime,
            );
          } else {
            this.logger.log(
              PerfLogChannel,
              'Passed finalization iteration, deltaTime: ' + deltaTime,
            );
          ...
          if (deltaTime < maxFrameLength) {
            this.logger.log(
              PerfLogChannel,
              'Passed initial check iteration, deltaTime: ' + deltaTime,
            );
            mainFractal.increaseFractalIterations(this.logger);
          } else {
            this.logger.log(
              PerfLogChannel,
              'Failed initial check iteration, deltaTime: ' + deltaTime,
            );
            mainFractal.reduceFractalIterations(this.logger);
        ...
        if (
          treeSpawnIntervals.length > 0 &&
          totalTimePassed >= this.timeBeforeSpawingTreesAndBushes
        ) {
        ...
        if (mainFractalCompleteAndCachedToBackground) {
          this.stars
            .filter(star => star.fadedIn && !star.drawIsCachedToBackground)
            .forEach(star => {
        ...
        this.trees
          .filter(tree => tree.growthDone && !tree.drawIsCachedToBackground)
          .forEach(tree => {
        ...
        this.bushes
          .filter(bush => bush.growthDone && !bush.drawIsCachedToBackground)
          .forEach(bush => {
        ...
        this.stars
          .filter(star => !star.drawIsCachedToBackground || star.twinkling)
          .forEach(star => {
            star.draw(context);
          });
  ...
  private getSpawnIntervals(
    width: number,
    numberToSpawnPer1000X: number,
    bufferZoneProportion: number,
  ): SpawnInterval[] {
