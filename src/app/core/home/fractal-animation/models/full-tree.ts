...
export class FullTree extends Tree {
  ...
  private normalPosition: Vector2 = {
    x: 0,
    y: 0,
  };
  ...
  constructor(
    ...
    this._lineLength = lodash.random(minLineLength, maxLineLength, false);
  ...
  update(deltaTime: number): void {
    ...
    this.totalDrawLengthIncludingInitialLine =
      this.lineLength * 4 * this.growthEasing(this.fractionGrowthDone);
    this._totalDrawLength = this.totalDrawLengthIncludingInitialLine - this.lineLength * 2;
  ...
  private drawInitialLine(context: CanvasRenderingContext2D): void {
    ...
    const initialLineLengthToDraw =
      this.totalDrawLengthIncludingInitialLine > this.lineLength * 2
        ? this.lineLength * 2
        : this.totalDrawLengthIncludingInitialLine;
    ...
    context.lineTo(
      this.position.x,
      this.normalPosition.y + this.lineLength * 2 - initialLineLengthToDraw,
    );
    context.stroke();
  }
}
