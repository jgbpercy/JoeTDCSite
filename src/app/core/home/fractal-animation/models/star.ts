...
export class Star extends Fractal {
  ...
  constructor(
    ...
    this._lineWidthChangePerFractalIteration = 0.1;
    this._initialLineWidth = this.lineWidthChangePerFractalIteration * 3;

    this._lineLength = lodash.random(3, 7, false);
    ...
    while (!foundPos) {
      ...
      if (
        Math.pow(xFromMainFractalCenter, 2) + Math.pow(yFromMainFractalCenter, 2) >
    ...
    this._color = new Color(lodash.random(150, 170, false), 150, lodash.random(120, 140, false), 0);

    this.normalAlpha =
      ((0.9 + 0.1 * lodash.random(0, 1, true)) * (canvasHeight - this.position.y)) / canvasHeight;

    this._drawLinesAtAngles = [0, Math.PI / 2, Math.PI, (3 * Math.PI) / 2];
  }
  ...
  update(deltaTime: number): void {
    if (!this.fadedIn) {
      ...
      if (this.twinkling) {
        ...
        const twinkleChanceInDeltaTime =
          1 - Math.pow(1 - this.twinkleChancePerSecond, Math.min(deltaTime, 0.07));
  ...
  drawNonTwinkling(context: CanvasRenderingContext2D): void {
    if (this.twinkling) {
