...
export class WindowSizeService {
  ...
  readonly windowWidthChange = this._windowWidthChange.asObservable();

  private readonly _windowHeightChange = new BehaviorSubject<number>(window.innerHeight);
  readonly windowHeightChange = this._windowHeightChange.asObservable();

  private readonly _windowSizeChange = new BehaviorSubject<WindowSize>({
    width: window.innerWidth,
    height: window.innerHeight,
  });
  readonly windowSizeChange = this._windowSizeChange.asObservable();

  private readonly _isMobileWidth: BehaviorSubject<boolean>;
  readonly isMobileWidth: Observable<boolean>;
  ...
  constructor() {
    ...
    widthStream.pipe(debounceTime(100)).subscribe(res => {
    ...
    heightStream.pipe(debounceTime(100)).subscribe(res => {
      this._windowHeightChange.next(res);
    });

    combineLatest(widthStream, heightStream)
